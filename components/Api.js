...
const loggedOutHeaders = () => ({'Content-Type': 'application/json', 'Accepts-version': `${apiVersion}`})
const loggedInHeaders = (sessionToken) => ({'Content-Type': 'application/json', 'Authorization': `Bearer ${sessionToken}`, 'Accepts-version': `${apiVersion}`})
...
const Api = {
  ...
  checkSessionToken: async(sessionToken, sessionId) => {
    try {
      let response = await fetch(
        `${baseUrl}/api/v1/sessions/${sessionId}/session_tokens`, {method: 'GET', headers: loggedInHeaders(sessionToken), }
      );
      return await response;
    } catch (error) {
      console.error(error);
  ...
  createUser: async (email, password, firstName, lastName) => {
    data = {data: {type: "users", attributes: {email, password, first_name: firstName, last_name: lastName}}}
    try {
      let response = await fetch(
  ...
  createEmailVerification: async (user, email, emailVerification) => {
    data = {data: {type: "email_verifications", attributes: {unconfirmed_email: email}}, meta: {user_id: user.data.id, token: emailVerification}}
    try {
      let response = await fetch(
        `${baseUrl}/api/v1/email_verifications`, {method: 'POST', body: JSON.stringify(data), headers: loggedOutHeaders(),}
      );
      return await response.json();
    } catch (error) {
      console.error(error);
  ...
  verifyBankAccount: async (sessionToken, bankAccountVerificationId, amount1, amount2) => {
    const data = {data: {type: "bank_accounts", attributes: {}}, meta: {bank_account_verification_id: bankAccountVerificationId, amount_1: amount1, amount_2: amount2}}

    try {
      let response = await fetch(
        `${baseUrl}/api/v1/bank_accounts`, {method: 'POST', body: JSON.stringify(data), headers: loggedInHeaders(sessionToken),}
      );
      return await response.json();
    } catch (error) {
      console.error(error);
  ...
  createBankAccountVerification: async (sessionToken, account, routing) => {
    const data = {data: {type: "bank_account_verifications", attributes: {account_number: account, routing_number: routing}}}

    try {
      let response = await fetch(
        `${baseUrl}/api/v1/bank_account_verifications`, {method: 'POST', body: JSON.stringify(data), headers: loggedInHeaders(sessionToken),}
      );
      return await response.json();
    } catch (error) {
      console.error(error);
  ...
  getPendingBankAccountVerifications: async (sessionToken) => {
    try {
      let response = await fetch(
        `${baseUrl}/api/v1/bank_account_verifications?status=started,sent_to_bank`, {method: 'GET', headers: loggedInHeaders(sessionToken), }
      );
      return await response.json();
    } catch (error) {
      console.error(error);
  ...
  getBankAccounts: async (sessionToken) => {
    try {
      let response = await fetch(
        `${baseUrl}/api/v1/bank_accounts`, {method: 'GET', headers: loggedInHeaders(sessionToken), }
      );
      return await response.json();
    } catch (error) {
      console.error(error);
  ...
  createBankTransaction: async (sessionToken, bankAccountId, currency, amount, type) => {
    const data = {data: {type: "bank_transactions", attributes: {currency, amount, type}}}

    try {
      let response = await fetch(
        `${baseUrl}/api/v1/bank_accounts/${bankAccountId}/bank_transactions`, {method: 'POST', body: JSON.stringify(data), headers: loggedInHeaders(sessionToken),}
      );
      return await response.json();
    } catch (error) {
      console.error(error);
  ...
  getPendingBankTransactions: async (sessionToken, bankAccountId) => {
    try {
      let response = await fetch(
        `${baseUrl}/api/v1/bank_accounts/${bankAccountId}/bank_transactions?status=pending`, {method: 'GET', headers: loggedInHeaders(sessionToken), }
      );
      return await response.json();
    } catch (error) {
      console.error(error);
