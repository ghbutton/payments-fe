...
export default function CreateAccountScreen({navigation}) {
  ...
  const submit = async () => {
    if (state === "CREATE_USER") {
      ...
      const verification = await Api.createEmailVerification(user, email, emailVerification);
  ...
  const emailVerificationView = () => (
    <Content padder>
      <Text>We sent a email to verify it, please type in the code that was sent</Text>
      <Form>
        <Item>
          <Input placeholder="Verification code" value={emailVerification} onChangeText={handleEmailVerificationChange} />
        </Item>
