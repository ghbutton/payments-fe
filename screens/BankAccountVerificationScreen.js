...
export default function BankAccountVerificationScreen({route, navigation}) {
  ...
  const handleVerification = async () => {
    const response = await Api.verifyBankAccount(state.sessionToken, bankAccountVerification.id, amount1, amount2);
    ...
    <View>
      <Form>
        <Item>
          ...
          <Input keyboardType="numeric" placeholder="0" onChangeText={handleAmount1Change} />
        </Item>
        <Item>
          <Label>Amount #2</Label>
          <Input keyboardType="numeric" placeholder="0" onChangeText={handleAmount2Change} />
        </Item>
